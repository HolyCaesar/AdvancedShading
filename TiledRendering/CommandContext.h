...
public:
    ...
    void SetPipelineState(const DX12PSO& PSO);
...
public:
    ...
    void SetRootSignature(const DX12RootSignature& RootSig);
...
inline void GraphicsContext::SetRootSignature(const DX12RootSignature& RootSig)
{
...
inline void ComputeContext::SetRootSignature(const DX12RootSignature& RootSig)
{
...
inline void CommandContext::SetPipelineState(const DX12PSO& PSO)
{
    ID3D12PipelineState* PipelineState = PSO.GetPSO();
...
{
    ASSERT(VertexData != nullptr && IMath::IsAligned(VertexData, 16));

    size_t BufferSize = IMath::AlignUp(NumVertices * VertexStride, 16);
...
{
    ASSERT(IndexData != nullptr && IMath::IsAligned(IndexData, 16));

    size_t BufferSize = IMath::AlignUp(IndexCount * sizeof(uint16_t), 16);
