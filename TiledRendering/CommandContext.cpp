...
uint64_t CommandContext::Flush(bool WaitForCompletion)
{
    ...
    if (m_CurGraphicsRootSignature) m_CommandList->SetGraphicsRootSignature(m_CurGraphicsRootSignature);
    if (m_CurComputeRootSignature) m_CommandList->SetComputeRootSignature(m_CurComputeRootSignature);
    if (m_CurPipelineState) m_CommandList->SetPipelineState(m_CurPipelineState);
...
void CommandContext::CopySubresource(DX12Resource& Dest, UINT DestSubIndex, GpuResource& Src, UINT SrcSubIndex)
{
    FlushResourceBarriers();
