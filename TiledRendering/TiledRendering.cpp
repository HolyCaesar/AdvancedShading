...
void TiledRendering::LoadAssets()
{
	...
	// Create Depth Buffer
	m_depthBufferFinal.Create(L"RenderPassDepthBuffer", m_width, m_height, DXGI_FORMAT_D32_FLOAT);
	m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_depthBufferFinal.GetResource(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_DEPTH_WRITE));
	{
		...
		m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_depthBuffer.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
	}
...
void TiledRendering::PreDepthPass()
{
	...
	m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_depthBuffer.Get(), D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_DEPTH_WRITE));
	...
	// Transmit the state of the depth buffer from SRV to DSV
	m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_depthBuffer.Get(), D3D12_RESOURCE_STATE_DEPTH_WRITE, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
	ThrowIfFailed(m_commandList->Close());
...
void TiledRendering::PopulateCommandList()
{
	...
	//auto dsvHandle = m_DSVHeap->GetCPUDescriptorHandleForHeapStart();
	auto dsvHandle = m_depthBufferFinal.GetDSV();
	m_commandList->RSSetViewports(1, &m_viewport);
	m_commandList->RSSetScissorRects(1, &m_scissorRect);
	//m_commandList->OMSetRenderTargets(1, &rtvHandle, FALSE, &dsvHandle);
	m_commandList->OMSetRenderTargets(1, &rtvHandle, FALSE, &dsvHandle);
	...
	m_commandList->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr);
	//m_commandList->ClearDepthStencilView(dsvHandle, D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr);
	m_commandList->ClearDepthStencilView(dsvHandle, D3D12_CLEAR_FLAG_DEPTH, m_depthBufferFinal.GetClearDepth(), m_depthBufferFinal.GetClearStencil(), 0, nullptr);
	m_commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
