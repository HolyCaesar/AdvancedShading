...
LinearAllocatorType DX12LinearAllocatorPageManager::sm_AutoType = kGpuExclusive;

DX12LinearAllocatorPageManager::DX12LinearAllocatorPageManager()
{
...
DX12LinearAllocatorPageManager DX12LinearAllocator::sm_PageManager[2];

LinearAllocationPage* DX12LinearAllocatorPageManager::RequestPage()
{
...
void DX12LinearAllocatorPageManager::DiscardPages(uint64_t FenceValue, const vector<LinearAllocationPage*>& UsedPages)
{
    lock_guard<mutex> LockGuard(m_Mutex);
...
void DX12LinearAllocatorPageManager::FreeLargePages(uint64_t FenceValue, const vector<LinearAllocationPage*>& LargePages)
{
    lock_guard<mutex> LockGuard(m_Mutex);
...
LinearAllocationPage* DX12LinearAllocatorPageManager::CreateNewPage(size_t PageSize)
{
    ...
    ASSERT_SUCCEEDED(IGraphics::g_GraphicsCore->g_pD3D12Device->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE,
...
void DX12LinearAllocator::CleanupUsedPages(uint64_t FenceID)
{
...
DynAlloc DX12LinearAllocator::AllocateLargePage(size_t SizeInBytes)
{
...
DynAlloc DX12LinearAllocator::Allocate(size_t SizeInBytes, size_t Alignment)
{
    ...
    const size_t AlignedSize = IMath::AlignUpWithMask(SizeInBytes, AlignmentMask);

    if (AlignedSize > m_PageSize)
        return AllocateLargePage(AlignedSize);

    m_CurOffset = IMath::AlignUp(m_CurOffset, Alignment);
