...
////
	...
	float yaw = m_HorizontalLookSensitivity * panScale;
	float pitch = m_VerticalLookSensitivity * panScale;

	float forward = m_MoveSpeed * speedScale * (isKeyPressed['W'] ? deltaTime : 0.0f + isKeyPressed['S'] ? -deltaTime : 0.0f);
	float strafe  = m_StrafeSpeed * speedScale * (isKeyPressed['A'] ? deltaTime : 0.0f + isKeyPressed['D'] ? -deltaTime : 0.0f);
	float ascent  = m_StrafeSpeed * speedScale * (isKeyPressed['Q'] ? deltaTime : 0.0f + isKeyPressed['E'] ? -deltaTime : 0.0f);
