...
void GraphicsPSO::Finalize()
{
    ...
    bool firstCompile = false;
    {
        ...
        else PSORef = iter->second.GetAddressOf();
    }

    if (firstCompile)
    {
        ASSERT_SUCCEEDED(g_GraphicsCore->g_pD3D12Device->CreateGraphicsPipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)));
...
{
    // Make sure the root signatuer is finalized first
    m_PSODesc.pRootSignature = m_RootSignature->GetSignature();
    ASSERT(m_PSODesc.pRootSignature != nullptr);
    ...
    if (firstCompile)
    {
        ASSERT_SUCCEEDED(g_GraphicsCore->g_pD3D12Device->CreateComputePipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)));
