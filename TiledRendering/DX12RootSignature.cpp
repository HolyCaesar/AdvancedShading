...
void DX12RootSignature::DestroyAll(void)
{
	s_RootSignatureHashMap.clear();
}

void DX12RootSignature::InitStaticSampler(
	UINT Register,
...
void DX12RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    if (m_Finalized)
        return;
    ...
    for (UINT Param = 0; Param < m_NumParameters; ++Param)
    {
        ...
        bool firstCompile = false;
        {
            ...
            // Reserve space so the next inquiry will find that someone got h
            if (iter == s_RootSignatureHashMap.end())
            {
                RSRef = s_RootSignatureHashMap[HashCode].GetAddressOf();
                firstCompile = true;
            }
            else RSRef = iter->second.GetAddressOf();
        }
